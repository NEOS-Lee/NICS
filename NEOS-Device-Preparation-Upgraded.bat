...
echo ======================================
echo 1. Remove OEM Bloatware (HP, Lenovo, Dell)
...
:: ============================================
:: Task 1 - Remove OEM Factory Bloatware
:: ============================================
:removebloatwarescript
cls
echo ======================================
echo Remove OEM Factory Bloatware
echo ======================================
echo.

if not exist "%~dp0Remove-OEMFactoryBloatware.ps1" (
    echo ERROR: Remove-OEMFactoryBloatware.ps1 was not found beside this batch file.
    echo Keep the master PowerShell script in the same folder as this batch file.
    call :Log "OEM bloatware task failed: master removal script missing."
    pause
    goto mainmenu
)

echo Detecting targeted HP, Lenovo, Dell, and common factory software...
call :Log "Starting OEM bloatware detection."
"%_PowerShell%" -NoProfile -ExecutionPolicy Bypass -File "%~dp0Remove-OEMFactoryBloatware.ps1" -Mode Detect -Target Auto
set "_OemDetectRC=%errorlevel%"

if "%_OemDetectRC%"=="0" goto OemBloatwareClean
if "%_OemDetectRC%"=="1" goto RunOemBloatwareRemoval

echo.
echo WARNING: OEM bloatware detection failed with exit code %_OemDetectRC%.
call :Log "OEM bloatware detection failed with exit code %_OemDetectRC%."
choice /C YN /N /M "Run the removal script anyway? [Y/N]:"
if errorlevel 2 goto mainmenu

:RunOemBloatwareRemoval
echo.
echo Removing targeted OEM bloatware...
call :Log "Starting OEM bloatware removal."
"%_PowerShell%" -NoProfile -ExecutionPolicy Bypass -File "%~dp0Remove-OEMFactoryBloatware.ps1" -Mode Remove -Target Auto
set "_OemRemovalRC=%errorlevel%"

if "%_OemRemovalRC%"=="0" goto OemBloatwareSuccess
if "%_OemRemovalRC%"=="3010" goto OemBloatwareRebootRequired

echo.
echo ERROR: OEM bloatware removal did not finish cleanly.
echo PowerShell exit code: %_OemRemovalRC%
echo Review the output above and the logs in:
echo %_LogDir%
call :Log "OEM bloatware removal ended with exit code %_OemRemovalRC%."
pause
goto mainmenu

:OemBloatwareClean
echo.
echo No targeted OEM bloatware was detected for this computer.
call :Log "OEM bloatware detection completed: no targeted software found."
pause
goto mainmenu

:OemBloatwareSuccess
echo.
echo OEM bloatware removal completed and verification passed.
call :Log "OEM bloatware removal completed and verification passed."
pause
goto mainmenu

:OemBloatwareRebootRequired
set "_RebootRequired=1"
echo.
echo OEM bloatware removal completed successfully.
echo A restart is required to finish one or more removals.
call :Log "OEM bloatware removal completed; restart required."
pause
goto mainmenu
